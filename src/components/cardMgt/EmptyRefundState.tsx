...
type Props = {
  initiateModal?: boolean;
  title: string;
  subtitle: string;
  image?: string;
};

const EmptyRefundState = ({ initiateModal, title, subtitle, image }: Props) => {
  return (
    <div className="py-7 rounded-[20px] bg-white text-center  w-8/12">
      <div className="flex w-full justify-center mb-3">
        <img src={image ? image : emptySheet} alt="emptySheet" className="" />
      </div>
      <p className="font-semibold"> {title}</p>
      <p className="my-3 text-sm text-blueGray">{subtitle}</p>
